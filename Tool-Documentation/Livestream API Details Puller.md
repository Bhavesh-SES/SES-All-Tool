...
- **Normal** **Page** - flat table for the selected outlet (select rows, create/export/share URLs).
  ...
  - Header checkbox selects/unselects all _visible_ rows.
...
1.  Open **Normal** and choose Outlet at the top dropdown next to search box option.
2.  Use **Search** to narrow to the streams you want (optional).
...
6.  OR click **Share Playback URL** → copy link & send to colleague to open the same layout.
...
1.  Select multiple outlets on the left (the order matters — first becomes reference).
...
After nightly ingest, run Filters → DRM to confirm which streams are flagged. Export and send CSV to ingest team. Use Share URL to open a playback layout to eyeball a few failing streams.
...
QA wants to test 6 players in a 2×3 grid. Select 6 streams (Normal), choose headend HLS, create Share URL with rows=2&cols=3, send link to QA for quick test load.
...
- **403 / no data:** upstream API is IP-restricted or CORS-blocked.
  ...
  - For local testing use the local proxy (localhost:8000) and run it via pm2/systemd for reliability.
